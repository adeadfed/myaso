...
SPECIAL_VARIABLES = {'$true', '$false', '$args'}
...
def replace_with_random_names(text, names_to_replace, generate_name: Callable):
    for name in sorted(names_to_replace, key=len, reverse=True):
        rand_name = generate_name()
        text = re.sub(rf'{re.escape(name)}(?!\w)', rand_name, text)
        logger.debug('[+] Replacing: {} -> {}'.format(name, rand_name))
    return text
